...
!     READ LINK DATA
	...
	DO i = 1,LINKS
		...
		!write(*,*) 'Time',sysstat(ntimestep),'Link',i,'pol', iType,
    ! & LinkPolicyChg(iType,i)
		call cpu_time(timings(1))
...
!  INPUT
      !Kang modify 20100630
      !CHARACTER*80 FileName
      integer*2 PolicyGrp, Policy, IDLINK, LINK,iLossMethod
!  OUTPUT
      LOGICAL*1 success
      !COMMON:
      ! LEVAP_PTS(LINK),  LINK_FLOW(JJ,LINK),  LINK_EVAP(JJ,LINK)
...
!  Local
      integer*2 i, n, GroupRead, IDLinkRead, iDatafile, iPos
      INTEGER*2 CompType, CompTypeRead, PolicyRead
      CHARACTER*30 aVar
      CHARACTER*256 aLine
      INTEGER*4 L
!------------------------------------------------------------------------
...
!  INPUT
      !Kang modify 20100630
      !CHARACTER*80 FileName
      integer*2     GroupID, PolicyID, iReadSeq, IDLink
!  OUTPUT
      logical*1     success
      !COMMON:      LinkLoss(iReadSeq), LossMethod()
!  Local variables
      integer*2     i, j, iPos, iDatafile, comp_type
      integer*2     GroupIDRead, PolicyIDRead, IDLinkRead
      CHARACTER*256 aLine
      CHARACTER*20  aVar
      INTEGER*4 L
...
!  INPUT
      character*80 FileName
      integer*2 PolicyGrp,Policy,IDLink, iReadSeq
!  OUTPUT
      LOGICAL*1 success
      !COMMON: L_NRTRES(LINK), L_CI(), L_CL(), L_CN()
!  local
      integer*2 i, iDatafile, iPos, iMethod, NumParms, CompType
      INTEGER*2 GroupRead, PolicyRead, IDLinkRead, CompTypeRead
      CHARACTER*30 aVar
      CHARACTER*256 aLine
      INTEGER*4 L
!------------------------------------------------------------------------
...
!  CALL: None
	...
	implicit none
      ...
      INCLUDE 'LINK.INC'
!  INPUT
      integer*2 PolicyGrp, Policy, IDLINK, link
!  OUTPUT
      LOGICAL*1 success
      !COMMON:
      !GWFromVol(IAGMAX*IAGMAX), GWToVol(IAGMAX*IAGMAX),GWFlowFromTo()
!  Local
      integer*2 n , iDatafile, iPos
      INTEGER*2 PolicyRead, GroupRead, IDLinkRead
      CHARACTER*30 aVar
      CHARACTER*256 aLine
      INTEGER*2 iFromVol, iToVol
!------------------------------------------------------------------------
      success = .false.
      !for unit conversion
...
!******************************************************************
	Subroutine  ReadGWData ( iDatafile,PolicyGrp, Policy,
     &            IDLink, link, success)
! For a bi-directional link:
! Read transfer data
!  CALL: None
...
!     &	 FORM='formatted', ERR= 999)
      ...
      !Kang modify for improving performance
      L = 1
      IF(Groundwater_BL>0) L = Groundwater_BL      
      do WHILE (.TRUE.)
        IF(nBufferedLines>0) THEN
            IF(L>nBufferedLines) EXIT
            aLine = pFileData(L)
        ...
        if (Index(aLine, 'Groundwater:') >= 1) then
          IF(Groundwater_BL<0) Groundwater_BL = L - 1      
          iPos = INDEX(aLine, ':')
     ...
     &          GWLength(Link),GWWidth(Link)
            !unit conversion
			!write(*,*)'ReadGWData textread, link ',IDLink	
...
!  INPUT
      character*80 FileName
      integer*2 PolicyGrp, Policy, IDNode, Node
...
!  OUTPUT
      ...
      ! LAKEQ_PTS(NN), NODE_LAKEQ(I,NN)
!  local
      integer*2 i, n , GroupRead, IDNodeRead, iDatafile, iPos
      INTEGER*2 CompType, CompTypeRead, PolicyRead
      CHARACTER*30 aVar
      CHARACTER*256 aLine
      INTEGER*4 L
...
!  INPUT
      CHARACTER*80  filename
      integer*2     GroupID, PolicyID, iReadSeq, IDNode
!  OUTPUT
      logical*1     success
      !COMMON:      NODE_EVAP(iReadSeq)
!  Local variables
      integer*2     i, j, iPos, iDatafile, comp_type
      integer*2     GroupIDRead, PolicyIDRead, IDNodeRead
      CHARACTER*256 aLine
      CHARACTER*20  aVar
      INTEGER*4 L
...
!  INPUT
      character*80 FileName
      integer*2 PolicyGrp, Policy, Node, IDNode
...
! local
      integer*2 i, n ,m,AlinkIDOld, GroupIDRead, PolicyRead
      INTEGER*2 NodeIDRead, iDatafile, iPos,t_cons_node
      CHARACTER*30 aVar
      CHARACTER*256 aLine
      INTEGER*2 aLinkID
...
!  OUTPUT
      ...
      !DMD_TARG(NN), DMD_T_CO(NN)
!  local
      integer*2 GroupRead, IDNodeRead, iDatafile, iPos
      INTEGER*2 CompType, CompTypeRead, PolicyRead,EnvFlow
      CHARACTER*30 aVar
      CHARACTER*256 aLine
      INTEGER*4 L
!------------------------------------------------------------------------
...
!  OUTPUT
      ...
      !SUPLY_PTS(NN), SUPL_NODE(COUNT,NN), SUPL_FRAC(COUNT,NN)
!  local
      integer*2 i, j, n , GroupRead, IDNodeRead, iDatafile, iPos
      INTEGER*2 CompType, CompTypeRead, PolicyRead
      CHARACTER*30 aVar
      CHARACTER*256 aLine
      INTEGER*4 L
...
!  INPUT
      character*80 FileName
      integer*2 PolicyGrp, Policy, IDNode, PowerLink1, iPower
!  OUTPUT
      LOGICAL*1 success
      !COMMON:
      ! HPCAP(LNKMAX), ECONST(LNKMAX), PLANT_FACTOR(LNKMAX)
...
!  Local
      integer*2 i, j, iDatafile, iPos, iLinkSeq
      INTEGER*2 PolicyRead, GroupRead, IDNodeRead
...
!  Input:
      CHARACTER*80 FileName
      INTEGER*2 PolicyGrp,Policy,ResvGrp,IDNode
!  Output:
      LOGICAL*1 success
      !COMMON: Rule_PTS(ResvGrp), res_rule(*,Resv,ResvGrp)
!  Local:
      integer*2  GroupRead, NodeIDRead, PolicyRead
      INTEGER*2  i, j, n , iDatafile, iPos
      CHARACTER*30 aVar
      CHARACTER*256 aLine
      INTEGER*4 L
!------------------------------------------------------------------------
...
!  INPUT
      character*80 FileName
      integer*2 PolicyGrp, Policy, RuleGrp, iGrpSeq
!  OUTPUT
      LOGICAL*1 success
...
!  INPUT
!      CHARACTER*80  filename
!      integer*2     GroupID, PolicyID, iReadSeq, IDNode, iType
      !COMMON:      SysStat(P_NGauge)
!  OUTPUT
!      logical*1     success
      !COMMON:      GageID(*,iReadSeq), GageMultiplier(*,iReadSeq)
!  Local variables
!      integer*2     i, j, iPos, iDatafile, nGagesSys
!      integer*2     GroupIDRead, PolicyIDRead, IDNodeRead, nGagesRead
!      CHARACTER*256 aLine
