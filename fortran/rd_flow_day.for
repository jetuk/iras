...
!
!	 *************************************************************************************
      SUBROUTINE GetTotalFlow(flowTot,Success)	!110323 Evgenii took out iMonth bc variable passed in was global 
...
C
C Local Variables:
      integer*2 i, j, kg, nGagesSys,Rec
      real*4 flow(GagMax)
        ...
        if (Gage(i)) then
          do kg = 1, nGagesSys				      
            if (NodeID(i)==GageIDObs(kg)) then           !GageIDObs(i,iType) changed to GageIDObs(i), bcs no added flow 100108
              flowTot(i)=flow(kg)*GageUserUnit(kg)       !GageUserUnit(kg,itype) changed to GageUserUnit(kg) bcs no added flow 100108
              exit
            end if
          end do
        end if
...
!   !input:
!      integer*2  iRepeat, iType, iYear,iMonth,iDay
...
C  Local Variables:
!      integer*2 i, j, k, iDatafile, FlowYear, FlowMonth, FlowDay
!      integer*2 EndMonth, EndDay
!      CHARACTER*256 aLine
!      CHARACTER*1 Tab
!      CHARACTER*30 aVar
!      integer*2 iYearRead, iMonthRead, iDayRead, nRead
!      INTEGER*2 nCurrentDays, nDaysWanted, nGagesInFile
