...
!--------------------------------------------------------------
	success = .false.
	iChangefile = 262
      OPEN(UNIT=iChangefile, FILE=TRIM('iras.chg'), STATUS='old',
     &	 FORM='formatted', ERR= 999,ACTION='READ')
...
!----------------------------------------------------------------
	...
	!write(*,*) 'MaxLineForSpeed ',MaxLineForSpeed
	  ...
	  !write(*,*) 'nbufferedLines, after=', nBufferedLines
        ...
        IF(nGW>0) ALLOCATE (GWArray(nGW))             							!Added by Anthony 230112
        IF(nTransfer>0) ALLOCATE (TransferArray(nTransfer))                       !Added by Anthony 230112
        !set the file position to the beginning of file
