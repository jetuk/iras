...
C  Functions:
      INTEGER*2      FINTERP
C  Local variables:
      INTEGER*2      NI, NO, I, IN_INDEX, OUT_INDEX, ST
...
!-------------------------------------------------------------------------
     ...
     &         LinkPolicy0(Transfer0,Link),LinkID(Link),link,success)
      ...
      do i = 2, GWALO_PTS(LINK)
        if (ABS(GWFromVol(i)-AVAIL_WATER(1,GWPoints(1)))>0.01) then
